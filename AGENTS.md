...
- UI code must not directly access SQLite, local files, API keys, provider SDKs, Python scripts, document converters, or runtime caches.
...
- Storage: SQLite for MVP; SQL Server Express may be considered later for multi-user intranet deployment.
- Model calls: DeepSeek API through an OpenAI-compatible provider adapter.
- Retrieval: start with SQLite FTS / keyword retrieval and optional LLM reranking. Do not require embeddings until an approved embedding API exists.
